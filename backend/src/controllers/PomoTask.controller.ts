...
export const CreatePomoTaskController = async (c: Context) => {
  ...
  try {
    ...
    const newTask = await TaskModel.CreatePomoTask({
      Pomo_Task_Title: body.Pomo_Task_Title,
      Pomo_Task_Short: body.Pomo_Task_Short,
      Pomo_Task_Long: body.Pomo_Task_Long,
      Pomo_Task_Status: false,
      Pomo_Completed_Count: 0,
      Pomo_Target_Count: body.Pomo_Target_Count,
...
export async function updatePomoTaskController(c: Context) {
  ...
  try {
    ...
    const taskData = {
      ...
      Pomo_Task_Short: body.Pomo_Task_Short,
      Pomo_Task_Long: body.Pomo_Task_Long,
      Pomo_Task_Status: body.Pomo_Task_Status,
      Pomo_Completed_Count: body.Pomo_Completed_Count,
      Pomo_Target_Count: body.Pomo_Target_Count,
      session: body.SessionId
        ? { connect: { SessionId: body.SessionId } }
        : undefined,
    };
    ...
    if (!updatedTask) {
      return c.json(
        { success: false, message: "Task not found or no changes made" },
        404,
      );
    }
...
export async function completePomoTask(c: Context): Promise<Response> {
  ...
  try {
    ...
    const newStatus = existing ? !existing.Pomo_Task_Status : true;
    const task = await TaskModel.updatePomoTask(taskId, {
...
export async function incrementPomoCounter(c: Context): Promise<Response> {
  ...
  try {
    ...
    return c.json(
      {
        success: false,
        message: "Failed to increment pomodoro counter",
        error: String(error),
      },
      500,
    );
...
export async function resetPomoCounter(c: Context): Promise<Response> {
  ...
  try {
    ...
    return c.json(
      {
        success: false,
        message: "Failed to reset pomodoro counter",
        error: String(error),
      },
      500,
    );
...
export async function setPomoTargetCount(c: Context): Promise<Response> {
  ...
  try {
    ...
    return c.json(
      {
        success: false,
        message: "Failed to set target count",
        error: String(error),
      },
      500,
    );
