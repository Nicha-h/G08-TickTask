...
export async function updatedSessionController(c: Context) {
  ...
  try {
    ...
    const updatedSession = await SessionModel.updatePomo(
      sessionId,
      sessionData,
    );
    ...
    if (!updatedSession) {
      return c.json(
        { success: false, message: "Session not found or no changes made" },
        404,
      );
    }
...
export async function startSession(c: Context) {
  try {
    ...
    const activeSession = await SessionModel.getActiveSession(user.id);

    if (activeSession) {
      await SessionModel.updatePomo(activeSession.SessionId!, {
...
export async function updateRemainingTime(c: Context) {
  ...
  try {
    ...
    if (isNaN(remainingSeconds)) {
      return c.json(
        { success: false, message: "Invalid remaining seconds value" },
        400,
      );
    }
    ...
    return c.json(
      {
        success: false,
        message: "Failed to update remaining time",
        error: String(error),
      },
      500,
    );
